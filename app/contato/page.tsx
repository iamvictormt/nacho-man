...
import { MessageCircle, MapPin, Clock, Phone, ShoppingCart, HelpCircle, Handshake, ArrowRight } from "lucide-react"
    ...
    <main className="min-h-screen bg-background">
      ...
      <section className="py-16">
        <div className="mx-auto max-w-7xl px-4">
          {/* Popup blocked alert */}
          {popupBlocked && (
            <div className="mb-8 p-4 rounded-xl bg-purple-medium/10 border border-purple-medium/30 text-center space-y-2">
              ...
              <a
                ...
                className="inline-block text-sm font-bold text-lime underline hover:text-lime/80 transition-colors"
              >
            ...
            <div className="space-y-8">
                ...
                {topics.map((topic, i) => (
                  ...
                  >
                    <div className="h-10 w-10 rounded-xl bg-lime/10 border border-lime/20 flex items-center justify-center shrink-0">
                      <topic.icon className="h-5 w-5 text-lime" />
                    </div>
                    <div className="flex-1">
                      <p className="text-sm font-bold text-foreground group-hover:text-lime transition-colors">{topic.title}</p>
                      <p className="text-xs text-muted-foreground">{topic.description}</p>
                    </div>
                    <ArrowRight className="h-4 w-4 text-muted-foreground group-hover:text-lime transition-colors shrink-0" aria-hidden="true" />
                  </button>
                ))}
              </div>
              ...
              <div className="space-y-4 pt-4 border-t border-border/50">
                <div className="flex items-center gap-3">
                  <Phone className="h-4 w-4 text-lime shrink-0" aria-hidden="true" />
                  <div>
                    <p className="text-[9px] font-bold tracking-wider text-muted-foreground">WHATSAPP</p>
                    <p className="text-sm font-semibold text-foreground">{FORMATTED_WHATSAPP}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <Clock className="h-4 w-4 text-lime shrink-0" aria-hidden="true" />
                  <div>
                    <p className="text-[9px] font-bold tracking-wider text-muted-foreground">HORÁRIO</p>
                    <p className="text-sm font-semibold text-foreground">{BUSINESS_HOURS}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <MapPin className="h-4 w-4 text-lime shrink-0" aria-hidden="true" />
                  <div>
