...
const molhos = allProducts.filter(
  (p) => p.category === "SECO" && (p.subcategory === "Molhos" || p.subcategory === "Food Service")
)
    ...
    <main className="min-h-screen bg-background">
      ...
      <section className="relative py-16 bg-graphite border-b border-border/20 overflow-hidden">
        <div className="absolute inset-0">
          <div className="absolute top-0 left-0 h-80 w-80 rounded-full bg-red-500/5 blur-[120px]" />
        </div>
        <div className="relative mx-auto max-w-7xl px-4">
          ...
          <p className="text-muted-foreground mt-3 max-w-lg">
            Salsas artesanais com receitas autênticas mexicanas. Do suave ao extremamente picante. Versões consumer e food service.
          </p>
          <p className="text-xs text-lime font-bold mt-4">
            {molhos.length} produtos
          </p>
        </div>
      </section>
      ...
      <section className="py-12">
        <div className="mx-auto max-w-7xl px-4">
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
            {molhos.map((product) => (
              <MolhoCard key={product.slug} product={product} />
            ))}
          </div>
        </div>
      </section>
...
function MolhoCard({ product }: { product: Product }) {
  ...
  return (
    <div className="group relative bg-graphite rounded-2xl border border-border/30 overflow-hidden hover:border-lime/30 transition-all duration-300">
      ...
      <a href={`/produto/${product.slug}`} className="block">
        <div className="aspect-square bg-gradient-to-br from-red-900/10 to-lime/5 p-6 flex items-center justify-center">
          <img
            src={product.image}
            alt={product.name}
